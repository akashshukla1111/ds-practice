...
//        here triple first value is vertex , 2nd value is edgeWeight and 3rd value is pathSoFar
    ...
    public void primsAlgo(GraphNode graphNode, Integer src) {
        Map<Integer, Set<Node>> graph = graphNode.getGraph();
//        here triple first value is vertex , 2nd value is edgeWeight and 3rd value is pathSoFar
        PriorityQueue<Triple<Integer, Integer, String>> quSrcWt = new PriorityQueue<>((Comparator.comparingInt(Triple::getMiddle)));
        quSrcWt.add(Triple.of(src, -1, "" + src));

        int[] vertexVisited = new int[999];

        while (quSrcWt.size() > 0) {
            Triple<Integer, Integer, String> node = quSrcWt.poll();
            if (vertexVisited[node.getLeft()] == 1) {
                continue;
            }
            System.out.println("visited node " + node.getRight());
            Integer dest = node.getLeft();
            Set<Node> dests = graph.get(dest);
            for (Node dd : dests) {
                if (vertexVisited[dd.getDestination()] != 1) {
                    quSrcWt.add(Triple.of(dd.getDestination(),  dd.getWeight(), node.getRight() + dd.getDestination()));
                }
            }
            vertexVisited[node.getLeft()] = 1;
        }
    }
}
